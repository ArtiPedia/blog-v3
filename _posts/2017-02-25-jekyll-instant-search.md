...
Finding specific content inside a website can be hard if it doesn't have a search option. I had no search option before since I had very few articles. Eventually, it became hard for me to search a small paragraph in some article that I know I have it on the website.

* Do not remove this line (it will not be displayed) 
{:toc}


Currently, I'm using google custom search which does a pretty good job but it doesn't fetch posts while I'm typing like how the actual google.com does. I might change this in the future but for now I'm keeping it. I have a feeling that it might help in SEO(not sure if it does).

## Why a Jekyll blog needs search option?
If your Jekyll site has just 4 to 5 pages, then you will not need any search option. All those pages can be linked in a menu bar or in the footer.
...
Lunr is instant but heavy. Google Custom Search Engine is light but not instant(yet). Simple Jekyll Search is light as well as instant. Let's implement it in a Jekyll site.
{: .y}

Simple Jekyll Search is developed by [christian-fei](https://github.com/christian-fei){: target="_blank"}. It is the easiest instant search available for Jekyll so far. Credit also goes to [Alex Pearce](https://github.com/alexpearce) who posted [his idea](https://alexpearce.me/2012/04/simple-jekyll-searching/){: target="_blank"}{: rel="nofollow"} on how it can be achieved.
...
Create a file in the root of your Jekyll blog and name it ``search.json``. Now, copy below code to in it and save.
...
It is better to create a folder named ``js`` and keep this file inside it just to avoid any confusions.
{: .g}

### Create a Jekyll search page
This step isn't necessary. You can add the below code to your default layout. Why I avoid adding search on all pages is that the search option is not used by all users all the time. There is no reason to call the search script(which is quite big) on all the pages. This may affect your page load time.
...
Now your search is ready. Try to enter something in the input field and see if it fetches any result.
